...
TStr  Top, To, Snd, FlacExe;           // top dir, dest dir
                                       // soundDir under To, flac.exe pathed
File  LF;                              // log file
uword NFn;
TStr   Fn [1024];                      // SFZ fn list
char  KGot [128];                      // keys in preset

const enum SfzP {KY, TR, CT,   KL, KH,   VL, VH,   SB, SE, LO, LB, LE,
                 PA, PT,   NO,   NSFZP};
char *Lbl [NSFZP] = {
   "root", "tran", "tune",
   "key", "to",
...
// sometimes _vX_ based on VL,VH
   if ((sfz[0][VL] != 1) || (sfz[0][VH] != 127))
      StrAp (pre, StrFmt (s[0], "`sv`03d_", dm ? "_" : "", sfz[0][VH]));
...
// adjust sample points base off SB,SE,LB,LE,LO
   if (got [0][SE] != 'n')
      {i = sfz [0][SE];   if ((ulong)i < wv._len)  wv._end  = (ulong)i;}
   if (got [0][SB] != 'n')
      {i = sfz [0][SB];   if ((ulong)i < wv._len)  wv._bgn  = (ulong)i;}
   if (got [0][LE] != 'n')
      {i = sfz [0][LE];   if ((ulong)i < wv._len)  wv._lEnd = (ulong)i;}
   if (got [0][LB] != 'n')
      {i = sfz [0][LB];   if ((ulong)i < wv._len)  wv._lBgn = (ulong)i;}
   if (got [0][LO] != 'n')
...
{ StrArr a, b, c;
  ...
  bool   inC, no1;
  ulong  i, j, k, e;
  slong  t, t2, t3, cc [1024], ncc, ccv [1024];
  ubyte  in, inp;                      // level we're in;  in previously
  TStr   pa, dp, sm, ts, fni;
  slong  sfz [3][NSFZP];               // global, group, region levels of params
...
// look for 999 format gm progch # on pathless fn prefix
   StrCp (ts, fn);   Fn2Path (ts);
   inp = (ubyte)Str2Int (& fn [StrLn (ts)+1], & p);
...
// melo GM Snd: gmdir\gmsnd_sfzFnSuffix
      ...
      StrFmt (Snd, "`s_", MProg [inp-1]);   // ^^^ len of 3?
      StrCp (ts, & fn [StrLn (ts)+5]);      // chop off 999 prefix
   }
// melo unGM Snd: etc\sfzFn
   else {
      StrFmt (Snd, "etc\\");
      StrCp (ts, & fn [StrLn (Top)+1]);     // back to sfz dir\fn off Top
   }
...
//DBG("ts=`s", ts);
         if      (! MemCm (ts, "set_cc", e = 6)) {    // spaces killed still:/
            ...
            for (e = 0;  e < (ulong)ncc;  e++)  if (cc [e] == t)  break;
            if (e < (ulong)ncc)  ccv [e] = t2;
...
//______________________________________________________________________________
class ThrUnSFZ: public Thread {
public:
   ThrUnSFZ (Waiter *w): _w (w), Thread (w->wnd)  {}
private:
   Waiter *_w;
...
DBG("{ ThrUnSFZ::Go `s", _w->arg);
   ...
   // load dir's .SFZ files and mess with em to get .WAVs out best ya can
     ...
     File  f;
     StrArr a;
      StrCp (ts, Top);   StrAp (ts, " - UnSFZ");   App.SetTitle (ts);
...
DBG("} ThrUnSFZ::Go");                 // ok, you can close Pop
      return End ();
   }
};
...
//______________________________________________________________________________
class DlgUnSFZ: public Dialog {
public:
   DlgUnSFZ (char *arg): Dialog (IDD_WAIT, IDI_APP), _t (NULL)
                {StrCp (_w.arg, arg);}
  ~DlgUnSFZ ()  {delete _t;}
private:
   ThrUnSFZ *_t;
   Waiter    _w;
   void Open ()  {_w.Init (Wndo ());   _t = new ThrUnSFZ (& _w);}
